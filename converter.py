...
def obtener_configuracion_ffmpeg():
    # Detecta si estamos en el EXE compilado de Windows
...
class NeonConverter(ctk.CTk):
    ...
    def crear_popup(self, titulo, mensaje, es_error=False):
        ...
        # Parche para Linux (Evita el crash de ventana no visible)
        try:
    ...
    def hilo_analizar(self):
        ...
        threading.Thread(target=self.analizar_video, args=(url,)).start()
    ...
    def hook_progreso(self, d):
        ...
        if d['status'] == 'downloading':
            ...
            self.lbl_estado.configure(text="Procesando...", text_color="white")

    # --- LIMPIEZA TOTAL (INCLUYENDO JPG/WEBP) ---
    def limpiar_basura(self):
        ...
        # Lista ampliada de extensiones a borrar
        ...
        for archivo_rastreado in list(self.archivos_rastreados):
            try:
                # 1. Borrar archivo exacto
                if os.path.exists(archivo_rastreado):
                    os.remove(archivo_rastreado)

                # 2. Borrar por nombre base (para eliminar carátulas que yt-dlp genera aparte)
                base, _ = os.path.splitext(archivo_rastreado)

                for ext in extensiones_posibles:
                    # Variante 1: NombreBase.jpg
                    ruta_simple = base + ext
                    if os.path.exists(ruta_simple):
                        try: os.remove(ruta_simple)
                        except: pass

                    # Variante 2: NombreBase.mp4.part (yt-dlp a veces hace esto)
                    ruta_compuesta = base + ".mp4" + ext
                    if os.path.exists(ruta_compuesta):
                        try: os.remove(ruta_compuesta)
                        except: pass
            except: pass
    ...
    def proceso_descarga(self):
        ...
        if ruta_ffmpeg: opciones['ffmpeg_location'] = ruta_ffmpeg
